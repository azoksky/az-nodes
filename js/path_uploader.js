// Compact Path Uploader UI with autocomplete + dropdown browser
...
function joinPath(base, seg, sep){ if(!base) return seg; if(!seg) return base; const trailing = base.endsWith("\\")||base.endsWith("/"); return trailing? base+seg : base+(sep||"/")+seg; }
...
app.registerExtension({
  ...
  beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      destInput.type="text";
      destInput.placeholder="Destination (e.g. C:/Users/you/Downloads or ~/models)";
      ...
      // datalist (type-ahead suggestions)
      const list=document.createElement("datalist");
      const listId="az-dir-list-"+Math.floor(Math.random()*1e9);
      list.id=listId; destInput.setAttribute("list", listId);
      const destWidget = this.addDOMWidget("dest_dir","Destination Folder",destInput);
      destWidget.inputEl = destInput;
      destInput.after(list);

      // size reduction for DOM widget row
      destWidget.computeSize = () => [this.size[0]-20, 34];

      // debounce for /az/listdir
      let listTimer=null;
      const scheduleList =()=>{ if(listTimer) clearTimeout(listTimer); listTimer=setTimeout(fetchChildren,200); };

      const fetchChildren = async ()=>{
        const val = destInput.value.trim(); if(!val){ list.innerHTML=""; return; }
        try{
          ...
          if(data?.ok && data.folders?.length){
            ...
            for(const folder of data.folders){
              ...
              opt.value=full; opt.label=folder.name; list.appendChild(opt);
            }
          }
        }catch(e){ /* silent */ }
      };

      destInput.addEventListener("input", ()=>{ this.properties.dest_dir = destInput.value; scheduleList(); });
      destInput.addEventListener("change", ()=>{ this.properties.dest_dir = destInput.value; scheduleList(); });

      // ===== Quick dropdown browser (context menu) =====
      this.addWidget("button","Browse","Open", async (v,ctx,e)=>{
        const base = (destInput.value || "").trim() || "/";
        try{
          const resp = await api.fetchApi(`/az/listdir?path=${encodeURIComponent(base)}`);
          const data = await resp.json();
          if(!data?.ok || !data.folders?.length) { this._status = data?.error || "No sub-folders"; this.setDirtyCanvas(true); return; }
          const entries = data.folders.map(f=>({ content:f.name, value:joinPath(data.root, f.name, data.sep) }));
          const LG = window.LiteGraph;
          if(LG && LG.ContextMenu){
            new LG.ContextMenu(entries, {
              event: e, callback: (v)=>{ if(!v) return; destInput.value=v.value; this.properties.dest_dir=v.value; fetchChildren(); }
            });
          }else{
            // fallback: put first 10 into datalist and focus input
            list.innerHTML=""; for(const f of data.folders.slice(0,10)){ const opt=document.createElement("option"); opt.value=joinPath(data.root,f.name,data.sep); opt.label=f.name; list.appendChild(opt); }
            destInput.focus();
          }
        }catch(err){ this._status = "Browse failed"; this.setDirtyCanvas(true); }
      });

      // ===== File picker / Upload / Cancel =====
      this.addWidget("button","Choose File","Browse…",()=>{
        ...
        picker.onchange = ()=>{
          ...
          this._sent=0; this._progress=0; this._status="Ready"; this._savedPath=""; this.setDirtyCanvas(true);
        };
        picker.click();
      });
      ...
      this.addWidget("button","Upload","Start",async ()=>{
        ...
        const dest=(this.properties.dest_dir||"").trim(); if(!dest){ this._status="Please enter destination folder."; this.setDirtyCanvas(true); return; }
        if(this._xhr) return;

        const form=new FormData(); form.append("file", this._selectedFile, this._selectedFile.name); form.append("dest_dir", dest);
      ...
      this.addWidget("button","Cancel","Stop",()=>{ if(this._xhr){ this._xhr.abort(); this._xhr=null; this._status="Canceled"; this.setDirtyCanvas(true);} });

      // ===== layout & drawing =====
      this.size = [520, 300]; // more vertical space to prevent overlap
      this.onDrawForeground = (ctx)=>{
        const pad=10, w=this.size[0]-pad*2, barH=14, yBar=this.size[1]-pad-barH-4;

        // Saved path (top info line)
        if(this._savedPath){
          ctx.font="12px sans-serif"; ctx.textAlign="left"; ctx.textBaseline="bottom";
          ctx.fillStyle="#9bc27c"; ctx.fillText(`Saved: ${this._savedPath}`, pad, yBar - 52);
        }

        // File line
        if(this._filename){
          ctx.font="12px sans-serif"; ctx.textAlign="left"; ctx.textBaseline="bottom";
          ctx.fillStyle="#8fa3b7"; ctx.fillText(`File: ${this._filename} (${fmtBytes(this._total)})`, pad, yBar - 36);
        }
      ...
      // initial suggestions if prefilled
