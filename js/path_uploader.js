// Full Path Uploader UI: compact path input with live dropdown, file picker,
// upload with % + speed + ETA, cancel. Works with /az/listdir and /az/upload.
  ...
  beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      const fetchChildren = async () => {
        ...
        if (!val) { items = []; renderDropdown(); return; }
        try{
          ...
          if (data?.ok && data.folders) {
            items = data.folders.map(f=>({
              name: f.name,
              path: joinPath(data.root || val, f.name, data.sep)
            }));
          } else { items = []; }
      ...
      destInput.addEventListener("input", ()=>{ this.properties.dest_dir = destInput.value; scheduleFetch(); });
      ...
      destInput.addEventListener("blur", ()=>{ setTimeout(()=>{ dropdown.style.display="none"; }, 150); });
